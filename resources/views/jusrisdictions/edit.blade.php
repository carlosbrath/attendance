       ...
       <div class="row">
        <div class="col-md-12">
            <div class="grid simple form-grid">
                ...
                <div class="grid-body">
                    <form class="form-no-horizontal-spacing" method="post"  action ="{{route('jurs.update',$item->id)}}" id="form-condensed" novalidate="novalidate">
                        {{method_field('PUT')}}
                          {{csrf_field()}}
                            ...
                            <div class="col-md-8">
                                ...
                                <div class="row form-row">
                                    <div class="col-md-8">
                                        ...
                                        <input name="title" id="form3FirstName"  type="text" class="form-control" value="{{ $item->title }}" placeholder="Enter Title">
                                        @if($errors->any())
                                ...
                                <div class="row form-row">
                                    <div class="col-md-8 mb-2">
                                        ...
                                        {!! Form::select('regions', ['all'=>'All Regions']+$data['regions']->toArray(),$item->regions, ['class' => 'form-control','id'=>'region_id']) !!}
                                    </div>   
                                </div>
                                <div class="row form-row">
                                    <div class="col-md-8 mb-2">
                                        <label class="form-label"><strong>Zones</strong></label>
                                        {!! Form::select('zones', ['all'=>'All Zones']+$data['zones'],$item->zones, ['class' => 'form-control','id'=>'r_zone_id']) !!}
                                    </div>   
                                </div>
                                <div class="row form-row">
                                    <div class="col-md-8 mb-2">
                                        <label class="form-label"><strong>Branches</strong></label>
                                        {!! Form::select('branches', ['all'=>'All Branches']+$data['branches'],$item->branches, ['class' => 'form-control','id'=>'z_branch_id']) !!}
                                    </div>   
                                </div>
                            </div>
...
<script>
    ...
    var parent_departments = @php echo "'".$item->parent_departments."'"; @endphp;

    var parent_departments_arr = parent_departments.split(',');
    ...
    $("#all_departments").select2ToTree({treeData: {dataArr: all_depts_data}, maximumSelectionLength: 3});
        $("#all_departments").val(parent_departments_arr).trigger('change');
