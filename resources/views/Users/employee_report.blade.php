	...
	<div class="col-md-12">
		<div class="grid simple form-grid">
			<div class="grid-body no-border">
				<form class="form-no-horizontal-spacing" method="get" action="{{route('employee_report',  $user_id)}}" id="form-condensed" novalidate="novalidate">
					@csrf
...
if ($date_attendance->count() > 0)
{
    ...
    if ($night_shift == 1)
    {
        ...
        if(is_null($tcat_hist->$day_in_pointer)){ //its off day
    ...
    // if(employeeRegistraionComparision($i->format('Y-m-d') , $user_info_query[0]->reg_date )){   //employee registraion date comparing 
    //             $time_in = "--";
    //             $timein = "--";
    //             $hours = "--";
    //             $expected_hours = "--";
						...
						@if(count($checktimes)>$j)
							...
							@endphp
                                ...
                                <td> 
                                 ...
                                 {{--   @if(employeeRegistraionComparision($i->format('Y-m-d'),$user_info_query[0]->reg_date))
                                    --
                                   @else --}}
                                    <a  data-toggle="tooltip" data-placement="top" title="{{$devic_id}}">
                                        {{  date('H:i:s', strtotime($checktimes[$j])) }}
                                    </a>
                                 {{--   @endif --}}
...
?>
	 ...
	 <?php }  ?>
						...
						@else
                     @if(strtotime($hours)>strtotime($expected_hours))
                       <?php $overtime_array[]=$timecalculation_remarks ?>
					 <td>{{$timecalculation_remarks}} o/t </td>
					 @elseif(strtotime($hours) < strtotime($expected_hours))
                     <?php $hour_short_array[]=$timecalculation_remarks ?>
