...
@section('content')
    ...
    <a class="btn btn-success noPrint" style="float: right" id="print_report"><i class="fa fa-print"> </i> Print Full Report</a>
                ...
                <div class="grid-body ">
                    <div class="table-responsive">
                        <table id="dataexample" class="table dataTable display nowrap" aria-describedby="example3_info" >
                            ...
                            $t=date('d-m-Y');
                            $selected_date=date('Y-m-d',strtotime($rep_date));
                                    ...
                                    @if($attandance_reports->where('id',$emp->id)->count()>0)
                                        @if($flag=="present" ||  $flag=="all")
                                            @foreach($attandance_reports->where('id',$emp->id)->groupBy('id') as $rep)
                                                ...
                                                $unixTimestamp = strtotime($rep->first()->att_date);
                                                $day=date("l", $unixTimestamp);
                                                if($day=="Monday"){
                                                    $time_in=$rep->first()->tc_monday_in;
                                                }elseif($day=="Tuesday"){
                                                    $time_in=$rep->first()->tc_tuesday_in;
                                                }elseif($day=="Wednesday"){
                                                    $time_in=$rep->first()->tc_wednesday_in;
                                                }elseif($day=="Thursday"){
                                                    $time_in=$rep->first()->tc_thursday_in;
                                                }elseif($day=="Friday"){
                                                    $time_in=$rep->first()->tc_friday_in;
                                                }elseif($day=="Saturday"){
                                                    $time_in=$rep->first()->tc_satureday_in;
                                                }elseif($day=="Saturday"){
                                                    $time_in=$rep->first()->tc_sunday_in;
                                                }



                                                $min = date('i',strtotime($rep->first()->grace_time));
                                                $time_in = strtotime("+".$min." minutes", strtotime($time_in));
                                                $time_in =  date('H:i', $time_in);

                                                $time_in = date('Y-m-d H:i:s', strtotime("$rep_date $time_in"));
                                                $attendance_time_in=  date('H:i', strtotime($rep->first()->attendance_time));

                                                if($rep->first()->attendance_time > $time_in){
                                                    $total_late+=1; $Emp_Status='Late';
                                                }if($rep->first()->attendance_time  <= $time_in){
                                                    $total_present+=1;
                                                    $Emp_Status='Present';
                                                }
                                                ...
                                                @if($Emp_Status=='Late')
                                                    ...
                                                    <!--  <td style="background-color:#f35958; text-color: white"> {{ $loop->iteration }}</td> -->
                                                        ...
                                                        <?php $timein = $rep[0]->attendance_time; $timein= date('h:i:s a' ,strtotime($timein)); ?>
                                                        <td style="color:white">{{$timein}}</td>
                                                        @if($rep->count()>1)
                                                            <?php $timeout=  $rep->last()->attendance_time; $timeout= date('h:i:s a' ,strtotime($timeout)); ?>
                                                ...
                                                @elseif($Emp_Status=='Present')
                                                    ...
                                                    <!--  <td style="background-color:#0aa699; color: white"> {{ $loop->iteration }}</td> -->
                                                        ...
                                                        @endif
                                                        <td style="color:white;">{{$rep->first()->bps}}</td>
                                                        <?php $timein = $rep->first()->attendance_time; $timein= date('h:i:s a' ,strtotime($timein)); ?>
                                                        <td style="color:white;">{{$timein}}</td>
                                                        @if($rep->count()>1)
                                                            <?php $timeout=  $rep->last()->attendance_time; $timeout= date('h:i:s a' ,strtotime($timeout)); ?>
                                                            <td style="color: white">{{$timeout}}</td>
                                        ...
                                        @if($flag=="absent" || $flag=="all")
                                            ...
                                            <!-- <td style="background-color:#29252799;color:white;"> {{ $loop->iteration }}</td> -->
                                                ...
                                                elseif($weekday=="Friday") {
                                                    $friday_in = "";
                                                    if($day_off->where('id',$emp->id)->count()>0){
    ...
    <div class="row onlyPrint"  id="print_att_table" >
        <div class="col-md-12 col-lg-12 col-xs-12 col-sm-12">
            <div class="grid simple ">
                <div class="grid-body ">
                    <h3 class="page-title">
                    ...
                    <div >
                        <table class="table "  >
                            ...
                            <tbody role="alert" aria-live="polite" aria-relevant="all" id="print_att_table_body">
                            </tbody>
                        </table>
                    </div>
                </div>
    ...
    <br>
    <div class="row-fluid" id="at_status_report_summary">
        <div class="col-md-12">
            <div class="grid simple ">
                <div class="grid-title no-border">
                    <h4>Summary</h4>
