				...
				@endif
                    ...
                    @if(sizeof($checktimes) == 0)
                       ...
                       if($day_off->count()>0){
                        ...
                        $time_category_diff=strtotime($day_off->first()->time_out)-strtotime($day_off->first()->time_in);
                         $time_category_diff=date('H:i:s',$time_category_diff);

                      
                          //$check_time_category='day_off';	
                        }
                        ...
                        if(date('l', strtotime($i->format('Y-m-d')))=="Monday"){
	                         if(empty($day_off->first()->tc_monday_in)){
					   	    	 $pay= round($pay);
								 echo  "<td>".$pay." Rs</td>";	
								 $total_pay_count+= $pay;				   	      }
	                  	  }elseif(date('l', strtotime($i->format('Y-m-d')))=="Tuesday"){
	                         if(empty($day_off->first()->tc_tuesday_in)){
					   	    	 $pay= round($pay);
								 echo  "<td>".$pay." Rs</td>";
								   $total_pay_count+= $pay;
					   	      }
	                  	  }elseif(date('l', strtotime($i->format('Y-m-d')))=="Wednesday"){
	                         if(empty($day_off->first()->tc_wednesday_in)){
					   	    	 $pay= round($pay);
								 echo  "<td>".$pay." Rs</td>";
								   $total_pay_count+= $pay;
					   	      }
	                  	  }elseif(date('l', strtotime($i->format('Y-m-d')))=="Thursday"){
	                         if(empty($day_off->first()->tc_thursday_in)){
					   	    	 $pay= round($pay);
								 echo  "<td>".$pay." Rs</td>";
								   $total_pay_count+= $pay;
					   	      }
	                  	  }elseif(date('l', strtotime($i->format('Y-m-d')))=="Friday"){
	                         if(empty($day_off->first()->tc_friday_in)){
					   	    	 $pay= round($pay);
								 echo  "<td>".$pay." Rs</td>";
								   $total_pay_count+= $pay;
					   	      }
	                  	  }elseif(date('l', strtotime($i->format('Y-m-d')))=="Saturday"){
	                         if(empty($day_off->first()->tc_satureday_in)){
	                         ...
	                         if(empty($day_off->first()->tc_sunday_in)){
                    ...
                    @elseif(sizeof($checktimes) > 1) 
                        @if(strtotime($hours) < strtotime($expected_hours))
                         ...
                         <?php 
							...
							// echo $b/24; 
							    ...
							    $hour_rate =$emp_fixed_salary->first()->fixed_salary;
								$hrs=date('g',strtotime($expected_hours));
								$mins=date('i',strtotime($expected_hours));
								$short_hrs=date('g',strtotime($hours));
								$short_mins=date('i',strtotime($hours));
                        ...
                        @elseif(strtotime($hours) > strtotime($expected_hours))
                         <?php
                            ...
                            $overtime=$emp_fixed_salary->first()->hour_rate_salary/60;
                             

                            $over_time_hrs=date('H',strtotime($timecalculation_remarks)); 
                            $over_time_mins=date('i',strtotime($timecalculation_remarks));                 
                            $over_day_time=$over_time_hrs.'.'.$over_time_mins;
                            $overtime_pay=$overtime*$over_day_time;
