           ...
           <tbody role="alert" aria-live="polite" aria-relevant="all">
             ...
             @if($attandance_reports)
              ...
              if($rep->first()->attendance_time=='' || $rep->first()->time_in==''  ){
                   if($daily_leave_reports->where('id',$rep->first()->id)->count()>0)
                   {
                    
                       $emp_leave= $daily_leave_reports->where('id', $rep->first()->id);

                       if($emp_leave->first()->leave_type==2){

                             $Emp_Status='Leave';
                               $leave=$emp_leave->first()->title;
                   ...
                   elseif($rep->first()->attendance_time=="" && $rep->first()->check_dayoff!="" ){
         ...
         <tr style="background-color:#1a033a99;color:white">
           ...
           <td style="color:white;">--</td>
            ...
            <?php
                 ...
                 // $leave_name=  App\TimeCategory::find($rep->first()->leave_type_id);
                 // $leave_title  = $leave_name->title;
                  echo '<td style="color:white;">'.$leave.'</td>';
           ?>
             <td style="color:white;">--</td>
