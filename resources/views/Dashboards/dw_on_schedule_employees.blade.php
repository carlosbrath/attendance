    ...
    @if($department->total>0)
      <div class="col-md-4 p-2 card_main">
          <div class="card text-center">
            <a href="{{   route('dw-os-employees',['date'=>$data['c_date'],'department_id'=>$department->id,'selected_dmgs'=>$data['selected_dmgs']]) }}" title="Drill Down" target="_blank">
            ...
            @foreach($departments as $department)
              @if($department->total>0)
                <tr>
                  ...
                  <a href="{{ route('dw-os-employees',['date'=>$data['c_date'],'department_id'=>$department->id,'selected_dmgs'=>$data['selected_dmgs']]) }}">
                    {{ $department->name }}
                  </a>
                  </td>
                  <td class="v-align-middle"><span class="muted">
                  <a href="{{ route('dw-os-employees',['date'=>$data['c_date'],'department_id'=>$department->id,'selected_dmgs'=>$data['selected_dmgs'],'list'=>'yes']) }}">
                    {{ $department->total }}
                  </a>
                  </span>
                  </td>
