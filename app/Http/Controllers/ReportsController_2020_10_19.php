      ...
      //start Wednesday
           ...
           if(date('l', strtotime($rep_date))=="Wednesday"){
                ...
                // this is condition for main account having sub_account filter e.g main pbm like having pbm quetta,karachi,lahore,peshawar etc
                   ...
                   }  //  wednesday $request->subdepartment_id!=''  && $rep_date!='' && $request->status=='all'
                    if($request->subdepartment_id!=''  && $rep_date!='' && $request->status=='absent' ){
                    ...
                    if($search!=''  && $rep_date!='' && $request->status=='absent' ){
                     ...
                     if($search=='all'  && $rep_date!='' && $request->status=='absent' ){
                      ...
                      $attandance_reports = collect(DB::select( DB::raw($report_query_for_departmnent_adm)));
                        ...
                        inner join time_categories 
                        on time_categories.id=users.tcat_id
                        WHERE clients.id='".Session::get('dep_id')."' ";
                        $day_off = collect(DB::select( DB::raw($for_dayoff)));
                       $daily_leave= "SELECT  u.id,
                                  lr.leave_type_id,
                                  lr.leave_type,
         ...
         //start satureday
                       ...
                       $daily_leave= "SELECT  u.id,
                        ...
                        $daily_leave_reports = collect(DB::select( DB::raw($daily_leave)));
                          $for_dayoff="select users.id,users.name,users.tcat_id,
                                           ...
                                           inner join time_categories 
                                        on time_categories.id=users.tcat_id
                                  WHERE clients.id='".$request->search."'";
                        $day_off = collect(DB::select( DB::raw($for_dayoff)));
          ...
          return view('Reports.attandance_report',compact('attandance_reports','department','rep_date','emp_by_dep','daily_leave_reports','fetch_emp','attach_emp_detail','flag','holidays','sub_department','check_day','day_off','weekday'));  
		}   
         $attandance_reports='';
         $daily_leave_reports='';
         $emp_total_leave='';
         ...
         $day_off="";
         $weekday="";
          return view('Reports.attandance_report',compact('attandance_reports','flag','department','rep_date','emp_total_leave','check_employee_leave','emp_by_dep','daily_leave_reports','fetch_emp','attach_emp_detail','holidays','sub_department','check_day','weekday'));
   } //end report method
} 
