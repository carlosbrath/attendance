...
class ReportsController extends Controller
{
  public function reports(Request $request){
    ...
    if($request->has('generate_report')){
      ...
      $users_tcats_clause = "";
      if($request->has('tcats')){
        $users_tcats_clause = " and u.tcat_id in (".implode(",",$choosen_tcats).") ";
      }      
    ...
    $data = [
      ...
      'total_emp_not_reg_prev_date'=>$emp_not_reg_prev_date,
      'time_categories' => TimeCategory::where('department_id',Session::get('dep_id'))->where('type',1)->get()
    ];
    if($request->has('department') && $request->department!="all"){
      $dept_names = '';
      foreach(Client::whereIn('id',$request->department)->pluck('name') as $name){
          $dept_names.= $name.","; 
      }
        $data['choosen_department_name'] = $dept_names;
    }else{
        ...
        for ( $i = $startTime; $i <= $endTime; $i = $i + 86400 ) {
            foreach($users as $user){
                ...
                if(count($this_day_attendance) == 0){
                  ...
                  if($user->TimeCategory == null){ continue; } 
                  $tcat_expected_hours = abs((strtotime($thisDate." ".$user_tcat->$this_day_tout_pointer) - strtotime($thisDate." ".$user_tcat->$this_day_tin_pointer))/3600); 
                    ...
                    if($is_night_shift == 1){
                        ...
                        if($tcat_out_check == 'tc_saturday_out'){
                            $tcat_out_check = "tc_saturday_out";
                        }
                        ...
                        if($status == "attach"){
                          $user_attendance['expected_hours'] = abs($tcat_expected_hours);
                          $user_attendance['hours_worked'] = $tcat_expected_hours;
                          $user_attendance['late_hours'] = 0;
                          $user_attendance['early_hours'] = 0;
                        }                 
                        if($status == "sd-attach"){
                          $user_attendance['expected_hours'] = (abs($tcat_expected_hours)/2);
                          $user_attendance['hours_worked'] = (strtotime($out) - strtotime($in))/3600;
                          $user_attendance['late_hours'] = 0;
                          $user_attendance['early_hours'] = 0;
                        }                 
