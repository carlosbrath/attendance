...
class IndividualPayrollController extends Controller
{
    ...
    $emp_salary="select id,overtime_check,fixed_salary,hour_rate_salary from users where id='".$request->user_id."'";
    $data['emp_fixed_salary']=collect(DB::select( DB::raw($emp_salary)));
   
    $bonus="SELECT amount,date  FROM `employee_bonuses` WHERE date BETWEEN '".$request->from_date."' and '".$request->to_date."' and user_id= '".$request->user_id."' and status=1";
    $data['emp_bonus']=  collect(DB::select( DB::raw($bonus)));
    $deduction="SELECT amount,date  FROM `employee_bonuses` WHERE date BETWEEN '".$request->from_date."' and '".$request->to_date."' and user_id= '".$request->user_id."' and status=2";
    $data['emp_deduction']=  collect(DB::select( DB::raw($deduction)));
    ...
    $data['user_info_query'] = DB::table('users as u')
     ...
     return view('Reports.individual_payroll',$data);
	}	
