  ...
  public function dashboard(){
   ...
   // User::count();
      ...
      $sixth_day_of_week  = date_create('this week +5 days')->format('Y-m-d');
       ...
       $l=date('d', strtotime($sixth_day_of_week)); 
         ...
         $daily_present="SELECT  sum(IF(temp.attendance_time < temp.time_in,1,0)) as 'present'
                        from
                          (SELECT u.id, ADDTIME(CONCAT('".$date."' ,' ',tcat.time_in),'00:15') as time_in,att.datetime as attendance_time
                           ...
                           where  u.department_id = '".Session::get('dep_id')."' and date(datetime)='".$date."' and u.flag=1 and u.status=1 GROUP by u.id order by u.id,datetime) as temp";
          ...
          $daily_late_count = DB::select( DB::raw($daily_late));
            $daily_leave="SELECT count(1) as leave_title
                          FROM(
                           ...
                           WHERE u.department_id='".Session::get('dep_id')."' and u.flag=1 and u.status=1 AND lev_re.leave_date='".$date."' and att.datetime is null group by lev_re.user_id) as temp";
         ...
         $daily_attach= "SELECT count(1) as attach_count FROM attach_employees_details 
                       ...
                       INNER JOIN clients on clients.id= users.department_id
                        ...
                        AND clients.id='".Session::get('dep_id')."'";
        ...
        $users="select count(users.id) as user_id from users INNER JOIN clients on users.department_id =clients.id WHERE clients.id='".$dep_id."' and users.flag=1 and users.status=1";
    ...
    $first_day_query =   "SELECT  
                           ...
                           from
                             (SELECT u.id, ADDTIME(CONCAT('".$first_day_of_week."' ,' ',tcat.time_in),'00:15') as time_in,att.datetime as attendance_time
                              from users u INNER JOIN clients c on u.department_id= c.id 
                              inner join attendance att on att.user_id=u.id 
                              inner join time_categories tcat on tcat.id=u.tcat_id 
                              where c.parent_id = '".Session::get('dep_id')."' 
    ...
    $second_day_query =   "SELECT  sum(IF(temp.attendance_time > temp.time_in,1,0)) as 'late',
                                  sum(IF(temp.attendance_time < temp.time_in,1,0)) as 'present'
                           from
                             (SELECT u.id, ADDTIME(CONCAT('".$second_day_of_week."' ,' ',tcat.time_in),'00:15') as time_in,att.datetime as attendance_time
                              from users u INNER JOIN clients c on u.department_id= c.id 
                              inner join attendance att on att.user_id=u.id 
                              inner join time_categories tcat on tcat.id=u.tcat_id 
                              where c.parent_id = '".Session::get('dep_id')."' 
    ...
    $second_day_of_weekly_graph_absent=$all_users[0]->user_id-$second_day_of_weekly_graph_present_late;
      ...
      $third_day_query =   "SELECT  sum(IF(temp.attendance_time > temp.time_in,1,0)) as 'late',
                                  sum(IF(temp.attendance_time < temp.time_in,1,0)) as 'present'
                           from
                             (SELECT u.id, ADDTIME(CONCAT('".$third_day_of_week."' ,' ',tcat.time_in),'00:15') as time_in,att.datetime as attendance_time
                              from users u INNER JOIN clients c on u.department_id= c.id 
                              inner join attendance att on att.user_id=u.id 
                              inner join time_categories tcat on tcat.id=u.tcat_id 
                              where c.parent_id = '".Session::get('dep_id')."' 
    ...
    $third_day_of_weekly_graph_absent=$all_users[0]->user_id-$third_day_of_weekly_graph_present_late;
      ...
      $fourth_day_query =   "SELECT  sum(IF(temp.attendance_time > temp.time_in,1,0)) as 'late',
                                  sum(IF(temp.attendance_time < temp.time_in,1,0)) as 'present'
                           from
                             (SELECT u.id, ADDTIME(CONCAT('".$fourth_day_of_week."' ,' ',tcat.time_in),'00:15') as time_in,att.datetime as attendance_time
                              from users u INNER JOIN clients c on u.department_id= c.id 
                              inner join attendance att on att.user_id=u.id 
                              inner join time_categories tcat on tcat.id=u.tcat_id 
                              where c.parent_id = '".Session::get('dep_id')."' 
    ...
    $fifth_day_query =   "SELECT  sum(IF(temp.attendance_time > temp.time_in,1,0)) as 'present',
                                  sum(IF(temp.attendance_time < temp.time_in,1,0)) as 'late'
                           from
                             (SELECT u.id, ADDTIME(CONCAT('".$fifth_day_of_week."' ,' ',tcat.time_in),'00:15') as time_in,att.datetime as attendance_time
                              from users u INNER JOIN clients c on u.department_id= c.id 
                              inner join attendance att on att.user_id=u.id 
                              inner join time_categories tcat on tcat.id=u.tcat_id 
                              where c.parent_id = '".Session::get('dep_id')."' 
    ...
    if($fifth_day_of_weekly_graph[0]->late==''){
     ...
     $sixth_day_query =   "SELECT  sum(IF(temp.attendance_time > temp.time_in,1,0)) as 'present',
                                  sum(IF(temp.attendance_time < temp.time_in,1,0)) as 'late'
                           from
                             (SELECT u.id, ADDTIME(CONCAT('".$sixth_day_of_week."' ,' ',tcat.time_in),'00:15') as time_in,att.datetime as attendance_time
                              from users u INNER JOIN clients c on u.department_id= c.id 
                              inner join attendance att on att.user_id=u.id 
                              inner join time_categories tcat on tcat.id=u.tcat_id 
                              where c.parent_id = '".Session::get('dep_id')."' 
